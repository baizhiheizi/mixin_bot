...
### The full Rails integration layer (`require 'mixin_bot/rails'`)

Beyond the generators, the gem ships an optional runtime integration layer. It is opt-in and Rails stays a soft dependency:

```ruby
# config/application.rb, after Bundler.require
require 'mixin_bot/rails'
```

Or scaffold everything in one command and skip pieces as needed:

```bash
rails generate mixin_bot:install
rails generate mixin_bot:install --skip-notifications --skip-transfers
```

What each piece gives you:

- **Blaze message routing** — `MixinBot::Blaze::Router` is a callable that plugs into `config.blaze_handler`. Handlers live in `app/mixin/handlers`, receive a decoded `Message`, and reply over the HTTP message API (no WebSocket access needed):

  ```ruby
  # config/initializers/mixin_bot_blaze.rb (generated by `rails g mixin_bot:blaze`)
  Rails.application.config.to_prepare do
    MixinBot.configure do
      self.blaze_handler = MixinBot::Blaze::Router.new do
        on 'text', Mixin::Handlers::TextHandler       # 'text' => PLAIN_TEXT
  ...
  Pair with the Puma plugin (`plugin :mixin_blaze`) so messages are received inside the web process tree. Per-bot: `MixinBot::Blaze::Router.new(api: MixinBot.bot(:shop))`.

- **Outputs polling** — `rails g mixin_bot:outputs` creates a receipt table (unique per `bot_app_id` + `output_id` — the dedup gate) and an example processor in `app/mixin/processors`. The poller's resume cursor is derived from the receipts themselves (newest recorded output `sequence` per bot — unique, monotonic, and the key the outputs API paginates on) — no separate cursor table. Run `rails mixin_bot:poller` (standalone process; pass `[bot_name]` for a registered bot). Every output — inbound *and* outbound — is delivered to matching processors through your job queue; memos bridge lazily via the snapshot notification API. Processors implement `self.matches?(envelope)` + `#process` and should be idempotent (dispatch is at-least-once).

- **Outbound transfers** — `rails g mixin_bot:transfers` creates a `MixinTransfer` ledger with the state machine (`pending → broadcast → confirmed | failed | reconciling`). Creating a record enqueues the Safe pipeline with the record's `trace_id` as the request id — retries are network-deduplicated. A timeout after submission lands in `reconciling`; the recurring `MixinTransfers.ReconcileJob` confirms from the trace snapshot or re-sends safely.

- **Mixin notifications** — `rails g mixin_bot:notifications` adds `noticed` and an example notification with `deliver_by :mixin`. The channel (`MixinBot::Notifications::MixinChannel`) sends text/post/app-card messages with the configured bot's credentials; it loads only when Noticed is present.

Multiple bots in one process: register named bots, then reference them everywhere (`MixinBot.bot(:shop)`, `mixin_bot:poller[shop]`, `Router.new(api: ...)`, `deliver_by :mixin, bot: :shop`):

```ruby
MixinBot.register_bot :shop,
                      app_id: '...', session_id: '...',
                      session_private_key: '...', server_public_key: '...',
                      spend_key: '...'
```
