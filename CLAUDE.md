...
- **Outputs poller** (`lib/mixin_bot/outputs/`): standalone `mixin_bot:poller` rake task; receipt dedup with the resume cursor DERIVED from receipts (`max(output_created_at)` per `app_id` — no cursor table; `MemoryReceiptStore` in gem, `ReceiptModel` concern for generated `MixinOutput` AR model, which self-registers as `MixinBot::Outputs.receipt_store`); processors in `app/mixin/processors` (`Mixin::Processors::*` < `Outputs::Processor`, `matches?` + `process`); `Envelope` bridges memo/opponent/trace lazily via `create_safe_snapshot_notification`
